...
/**
 * Strip transitional kernelServices dependency once kernel is initialized.
 * 
 ...
 */
export function stripKernelServicesDependency(facetsByKind, hooksByKind) {
  for (const facet of Object.values(facetsByKind)) {
    if (facet.hasDependency?.('kernelServices')) {
      facet.removeDependency('kernelServices');
    }
  }
  // Also remove from hook metadata (now arrays per kind)
  for (const hookList of Object.values(hooksByKind)) {
    if (Array.isArray(hookList)) {
      for (const hookMeta of hookList) {
        const idx = hookMeta.required.indexOf('kernelServices');
        if (idx !== -1) hookMeta.required.splice(idx, 1);
      }
    }
  }
}
 ...
 * @param {Object} hooksByKind - Object mapping hook kinds to arrays of hook metadata
 * @param {Object} facetsByKind - Object mapping facet kinds to Facet instances
 * @param {BaseSubsystem} subsystem - Subsystem instance (for kernelServices check)
 */
  ...
  for (const [kind, hookList] of Object.entries(hooksByKind)) {
    ...
    for (const hookMeta of hookList) {
      for (const dep of hookMeta.required) {
        // Skip validation for kernelServices if kernel is not initialized
        // (it's a transitional dependency that may not have a facet)
        if (dep === 'kernelServices' && !subsystem?.ms?.isKernelInit?.()) {
          continue;
        }

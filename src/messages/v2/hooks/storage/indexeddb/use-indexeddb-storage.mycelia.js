 ...
 * 
 * Provides IndexedDB-based storage functionality to subsystems.
 * Uses IndexedDBStorageBackend for persistent, browser-based storage.
 * 
 ...
 */
import { IndexedDBStorageBackend } from './indexeddb-storage-backend.mycelia.js';
import { Facet } from '../../../models/facet-manager/facet.mycelia.js';
import { createHook } from '../../create-hook.mycelia.js';
import { getDebugFlag } from '../../../utils/debug-flag.utils.mycelia.js';

export const useIndexedDBStorage = createHook({
  kind: 'storage',
  version: '1.0.0',
  overwrite: false,
  required: [],
  ...
  fn: (ctx, api, subsystem) => {
    ...
    // Create IndexedDB storage backend
    // Database will be initialized lazily on first use
    const backend = new IndexedDBStorageBackend({
      dbName: config.dbName || 'mycelia-storage',
      dbVersion: config.dbVersion || 1,
      migrate: config.migrate !== false,
      debug: getDebugFlag(config, ctx)
    });
    
    // Initialize database asynchronously (don't block hook execution)
    // Backend will ensure initialization before first use
